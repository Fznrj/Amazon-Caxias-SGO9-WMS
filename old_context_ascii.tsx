 ...
 * ???????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????
 * Motor do sistema de gamifica????o da opera????o.
 * Gerencia XP, SPR, Ranking di??rio/mensal e Conquistas.
 *
 * FONTES:
 *  - useKpi() ??? operatorProductivity (ranking de scans v??lidos)
 *  - useWmsData() ??? logs brutos APENAS para erros e m??tricas de conquistas
 *
 * PROIBIDO:
 *  - Fazer SELECT/fetch direto no Supabase para tabelas de log
 *  - Toda mat??ria-prima vem de useKpi e useWmsData
 *
 * REATIVIDADE:
