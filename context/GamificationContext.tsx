...
export const GamificationProvider: React.FC<GamificationProviderProps> = ({ children, currentUser }) => {
    ...
    const { fetchProductivityReport, operatorProductivity } = useKpi();
    ...
    const operatorMetrics = useMemo(() => {
        ...
        const ensureOp = (op: string) => {
            if (!metrics.has(op)) {
                metrics.set(op, {
                    ...
                    dailyActivities: {}, dailyErrors: {}, todayTimestamps: []
                });
            }
            return metrics.get(op)!;
        };

        const pushTimestamp = (data: any, dateKey: string, timeStr?: string) => {
            if (dateKey === todayKey && timeStr) {
                const ts = new Date(timeStr).getTime();
                if (!isNaN(ts)) data.todayTimestamps.push(ts);
            }
        };
        ...
        inboundItems.forEach(item => {
            ...
            pushTimestamp(data, dateKey, item.createdAt || item.time);
        ...
        [...rtsItems, ...rtsLogs].forEach(item => {
            ...
            pushTimestamp(data, dateKey, item.created_at || item.time);
    ...
    const recalculateAll = useCallback(async () => {
        ...
        try {
            ...
            const todayReports = operatorProductivity || [];
            ...
            const processOperator = async (operatorName: string, todayScans: number) => {
                ...
                const profile = await gamificationService.recalculate(
                    ...
                    currentUser,
                    {
                        ...
                        todayTimestamps: metrics?.todayTimestamps || []
                    },
    ...
    }, [currentUser, initialized, operatorMetrics, fetchProductivityReport, operatorProductivity]);
